...
from torchocr.config.yacs import CfgNode as _CfgNode
...
class CfgNode(_CfgNode):
    """
    Our own extended version of :class:`utils.config.CfgNode`.
    ...
    def load_yaml_with_base(filename: str, allow_unsafe: bool = False) -> None:
        ...
        if BASE_KEY in cfg:
            ...
            if base_cfg_file.startswith("torchocr://"):
                base_cfg_file = get_config_file(os.path.split(base_cfg_file)[-1])
...
def get_config_file(config_path):
    ...
    Args:
        config_path (str): config file name relative to torchpack's "configs/"
    ...
    cfg_file = pkg_resources.resource_filename(
        "torchocr.config", os.path.join("base", config_path)
    )
