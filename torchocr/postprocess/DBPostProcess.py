...
#         bitmap = _bitmap
#         height, width = bitmap.shape
#         available_region = np.zeros_like(bitmap, dtype=np.float32)
#         np.putmask(available_region, bitmap > 0, bitmap)
#
#         bitmap = (bitmap > 0).astype(np.uint8) * 255
#         structure_element = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
#         bitmap = cv2.morphologyEx(bitmap, cv2.MORPH_CLOSE, structure_element)
...
#             cv2.drawContours(m_available_mask, [contour, ], 0, 255, thickness=-1)
...
#             score = float(np.sum(m_region_mask) / m_mask_count)
#
#             # score = self.box_score_fast(pred, points.reshape(-1, 2))
#             if self.box_thresh > score:
#                 continue
